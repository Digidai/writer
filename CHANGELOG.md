...
移动端设置分段在满宽轨道上均分：每个 `.segment` 为 `flex: 1`，标签居中，右侧不再留空槽。选中态仍是书桌色轨道上抬起的纸面。静态资源版本参数升级到 `?v=0.12.2`。

## [0.12.1] - 2026-08-18

设置分段不再墨色反白：未选透明、`--ink-soft`；选中是书桌色轨道上抬起的纸面。归档检索模式按钮同样处理。去掉 `.setting-desc` 的 nowrap/省略号，避免中文被裁切。静态资源版本参数升级到 `?v=0.12.1`。

## [0.12.0] - 2026-08-17

视觉系统：安静书桌。编辑器、阅读页、归档行与设置共用一张纸（`--paper`、1px 发丝线、3px 圆角、短落桌阴影），去掉卡片悬停抬起。设置收进一张纸，分组内凹，回收站在纸底。桌面纸面 794×1123，行宽约 32–36em，字距 0，行高约 1.80；阅读正文用 `var(--doc-size)`。顶栏实色书桌、无毛玻璃，高度约 56px；完成在桌面是安静文字；菜单用 `--ink-soft`；桌面热区约 32px，触控 44px。字重 500 全部改为 400。提高 `--ink-faint`，暗色纸面约 `#242119`，选区随主题。归档改为密行，检索圆角 3–4px。分段控件始终一颗胶囊。设置说明压成一行。`:focus-visible` 印章描边只加在控件上。收紧底部围裙。删除失效的 `.doc-meta .download`。移动端 0.11 舞台不变。静态资源版本参数升级到 `?v=0.12.0`。
...
修复移动端书写纸面：改为弹性填充布局，移除 `--bar-height` 与 `--safe-top` 重复计算，恢复可见页边距并让输入高度至少等于可见纸面；同时修正触屏下「完成」按钮提示不再显示 `⌘⏎`。

## [0.9.2] - 2026-08-17

修复移动端空白草稿首屏：纸面改为填满可写区域、补全栏仅在可见时预留底部空间，且无建议时提示不再提及「采纳」。
